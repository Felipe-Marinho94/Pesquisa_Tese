...
#Regularização de Tichonov
K_tilde = K + (1/1000)*np.diag(np.full(K.shape[0], 1))
...
#Gradiente
Gradient = np.random.normal(loc = 0, scale=1, size=100)
...
while k <= 10:
        ...
        #Realizando as atualizações
        r = (t[i] - t[j])/tau
        s = np.eye(1, 100, i) - np.eye(1, 100, j) + r * s
        t = K_tilde[:, j] - K_tilde[:, i] + r * t
        tau = t[j] - t[i]
        ...
        #Condição de parada
        if (np.argmax(Gradient) - np.argmin(Gradient)) < 0.01:
            break
